...
\subsubsection{Illegal Fixture Tags}
There are some tags which cannot be used for fixtures. This is because they 
interfere with the pseudo-tags that Pylux uses to speed up the insertion of 
data on the command line.

\paragraph{\texttt{position}}
Used to define \texttt{posX} and \texttt{posY} at the same time.

\paragraph{\texttt{focus}}
Used to define \texttt{focusX} and \texttt{focusY} at the same time.
...
is documented here anyway for ease of access.
...
\subsection{The \texttt{clihelper} module}
...
generate an interface like that generated by \texttt{curses}, it mainly assists 
with the processing of on-screen references.
